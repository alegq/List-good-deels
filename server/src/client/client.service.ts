...
export class ClientService {
  ...
  async create(CreateClientDto: CreateClientDto): Promise<Client> {
    ...
    const existLogUser = await await this.ClientModel.find({
    ...
    const existTegUser = await await this.ClientModel.find({
      teg: CreateClientDto.teg,
    });
    if (existTegUser[0]) {
      errorMesseng[0]['teg'] = 'This TEG already exist!'; //добавляем ошибку в общий массив ошибок
    }

    if (Object.keys(errorMesseng[0]).length != 0)
      throw new BadRequestException(errorMesseng); // если хоть одна ошибка есть, пораждаем error
  ...
  async getOneLog(logPasswDto: LogPasswdDto): Promise<any[]> {
    const user = await this.ClientModel.find({
      login: logPasswDto.login,
      password: logPasswDto.password,
    });
    return user;
  }

  async getFriend(teg: string): Promise<any[]> {
    const friend = await this.ClientModel.find({ teg: teg });
    return friend;
  }
